...
if($whereLOGadmin_viewable_groupsSQL == "")
	$whereLOGadmin_viewable_groupsSQL = "where user_level = 1";
...
if ( (preg_match('/\-\-ALL\-\-/',$user_group_string) ) or ($user_group_ct < 1) )
	...
	$usergroup_SQL = "and user_group IN($user_group_SQL)";
...
if ( (preg_match('/\-\-ALL\-\-/',$user_string) ) or ($user_ct < 1) )
	...
	$userSQL = "and Usuario IN($user_SQL)";
...
$report_name = "Estado de conexión por agente";
...
$HTML_text.="<b>"._QXZ("$report_name")."</b>\n";
...
$HTML_text.="<input type='hidden' name='live_agents' value='checked' $live_agents>"/*._QXZ("Only live agents")*/."<BR>\n";
$HTML_text.="<input type='hidden' name='show_percentages' value='checked' $show_percentages>"/*._QXZ("Show %s")*/."<BR>\n";
$HTML_text.="<input type='hidden' name='time_in_sec' value='checked' $time_in_sec>"/*._QXZ("Time in seconds")*/."<BR>\n";
$HTML_text.="<input type='hidden' name='breakdown_by_date' value='checked' $breakdown_by_date>"/*._QXZ("Show date breakdown")*/."<BR>\n";
$HTML_text.="<input type='hidden' name='search_archived_data' value='checked' $search_archived_data>"/*._QXZ("Search archived data")*/."<BR>\n";
$HTML_text.="<input type='hidden' name='show_defunct_users' value='checked' $show_defunct_users>"/*._QXZ("Show defunct users")*/."\n";
$HTML_text.="</TD><TD VALIGN=TOP>";
...
$HTML_text.="<option value='TEXT'>"._QXZ("TEXT")."</option>";
// $HTML_text.="<option value='HTML'>"._QXZ("HTML")."</option>";
$HTML_text.="</select><BR>\n";
$HTML_text.="<input type='hidden' name='shift' value='--'>"/*._QXZ("Only live agents")*/."<BR>\n";
$HTML_text.="<input type='hidden' name='file_download' value='3'>"/*._QXZ("Only live agents")*/."<BR>\n";
...
if ($file_download == 0 || !$file_download) 
    ...
    /************* Empezado por William */

if ($file_download == 3)
{  
	$fileoutput = "";
	$i=0;
	$stmt="DESCRIBE report_csv2";
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "|$stmt\n";}
	$numRegistry = mysqli_num_rows($rslt);
	while ($i < $numRegistry)
	{
		$row=mysqli_fetch_row($rslt);
		$fileoutput .= $row[0];
	...
	$fileoutput .= "\n";
	$i=0;
	$stmt="SELECT * FROM report_csv2 WHERE $groupSQL $usergroup_SQL $userSQL and Inicio >= '$query_date_BEGIN' and fin <= '$query_date_END'";
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "|$stmt\n";}
	$numRegistry = mysqli_num_rows($rslt);
	while ($i < $numRegistry)
	...
	$FILE_TIME = date("Ymd-His");
	$CSVfilename = "Estado_de_conexion_por_agente_$US$FILE_TIME.csv";
