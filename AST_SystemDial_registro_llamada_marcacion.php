...
if ( (preg_match('/\-\-ALL\-\-/',$user_group_string) ) or ($user_group_ct < 1) )
	...
	$TCusergroupSQL = "and user_group IN($TCuser_group_SQL)";
...
if ($file_download < 1)
	...
	echo "<TITLE>Registro llamada marcacion</TITLE></HEAD><BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>\n";
	...
	echo "<b>Registro llamada marcacion</b>\n";
...
#	echo "\n";
    ...
    if ($file_download > 0)
    {
        ...
        $fileoutput .= "\n";
        $i=0;
        $stmt="SELECT * FROM report_csv1 WHERE $groupSQL $TCusergroupSQL and FechaInicio_Llamada between '$query_date_BEGIN' and '$query_date_END'";
        $rslt=mysql_to_mysqli($stmt, $link);
        if ($DB) {echo "|$stmt\n";}
        $numRegistry = mysqli_num_rows($rslt);
        while ($i < $numRegistry)
        ...
        $CSVfilename = "REGISTRO_LLAMADA_MARCACION_$US$FILE_TIME.csv";
...
echo "</SELECT><BR>\n";
echo "<input type='hidden' name='file_download' value='1'";
echo "<input type='hidden' name='show_parks' value='checked' $show_parks>"./*_QXZ("Show parks-holds").*/"<BR>";
echo "<input type='hidden' name='time_in_sec' value='checked' $time_in_sec>"./*_QXZ("Time in seconds").*/"<BR>";
echo "<input type='hidden' name='search_archived_data' value='checked' $search_archived_data>"./*_QXZ("Search archived data").*/"<BR>\n";
...
echo "<option value='TEXT'>"._QXZ("TEXT")."</option></select>\n<BR><BR>";
echo "<INPUT TYPE=SUBMIT NAME=SUBMIT VALUE='"._QXZ("DOWNLOAD")."'>\n";
