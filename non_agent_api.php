...
if ($function == 'records_list_api')
{
	...
	if(isset($_GET["fecha_inicio"])){
		$fechaini = "";
		if($_GET["fecha_inicio"] != "all")
			$fechaini = " and rl.`start_time` >= '".$_GET['fecha_inicio']."'";
	}
	if(isset($_GET["fecha_final"])){
		$fechafin = "";
		if($_GET["fecha_final"] != "all")
			$fechafin = " and rl.`end_time` <= '".$_GET['fecha_final']."'";
	}
	if(isset($_GET["tipificacion"])){
		$tipificacion = " and val.`status` != ''";
		if($_GET["tipificacion"] != "all")
			$tipificacion = " and lower(val.`status`) = lower('".$_GET['tipificacion']."')";
	}
...
################################################################################
### deletesession_agent - delete the sesion of the agent
################################################################################
if ($function == 'deletesession_agent')
{	
	$stmt = "DELETE FROM vicidial_session_data where user='".$_GET['agent']."'";
	$rslt=mysql_to_mysqli($stmt, $link);
	$stmt = "DELETE FROM vicidial_live_agents where user='".$_GET['agent']."'";
	$rslt=mysql_to_mysqli($stmt, $link);
	$stmt = "DELETE FROM vicidial_live_inbound_agents where user='".$_GET['agent']."'";
	$rslt=mysql_to_mysqli($stmt, $link);
	exit;
}
################################################################################
### END deletesession_agent
################################################################################


################################################################################
### callbacks_agent - sends a list of the sounds in the audio store
################################################################################
if ($function == 'callbacks_agent')
	{
		Header ("Content-type: text/json; charset=utf-8");		
		$stmt = "SELECT * FROM vicidial_callbacks vc LEFT JOIN vicidial_list vl ON vl.lead_id = vc.lead_id where vc.recipient='USERONLY' and vc.user='".$_GET['agent']."' and vc.campaign_id = '".$_GET['campaign']."' and vc.status NOT IN('INACTIVE','DEAD') order by vc.callback_time";
		$rslt=mysql_to_mysqli($stmt, $link);
		if ($DB) {echo "$stmt\n";}
		$results = mysqli_num_rows($rslt);
		$i = 0;
		if($results > 0){
			...
			header($_SERVER["SERVER_PROTOCOL"]." 404 Not Found");// echo "{'msg':'No callbacks'}";
		}
		...
		if ($allowed_user < 1)
			...
			if ($search_ready < 1)
				...
				if (strlen($time_format) < 1)
					{$time_format = 'HF';}
				if ($header == 'YES')
					{$output .= 'status' . $DL . 'callerid' . $DL . 'lead_id' . $DL . 'campaign_id' . $DL . 'calls_today' . $DL . 'full_name' . $DL . 'user_group' . $DL . 'pause_code' . $DL . 'user_level' . $DL . 'real_time_sub_status' . $DL . 'phone_number' . $DL . 'vendor_lead_code' . $DL . 'session_id' . "\n";}
