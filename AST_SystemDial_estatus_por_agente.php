...
$report_name = "Estatus por agente";
...
{  
	...
	$fileoutput .= "\n";
	$i=0;
	$stmt="SELECT * FROM report_csv3 WHERE $groupSQL $usergroup_SQL $userSQL and Inicio >= '$query_date_BEGIN' and fin <= '$query_date_END'";
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "|$stmt\n";}
	$numRegistry = mysqli_num_rows($rslt);
	while ($i < $numRegistry)
	...
	$FILE_TIME = date("Ymd-His");
	$CSVfilename = "Estatus_por_agente_$US$FILE_TIME.csv";
